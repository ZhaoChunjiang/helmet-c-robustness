...
3. V1.0.4 使用 8 个 CPU 进程并行生成退化图像 + GPU 小批次推理；
   默认 batch=8，若 CUDA OOM 自动 8 -> 4 -> 2 -> 1。
4. 每个 corruption/severity 独立缓存。即使中途关机，再运行会自动从未完成条件继续。
5. 不要升级/更改 A0 已冻结的 PyTorch / Ultralytics / NumPy 环境。\n6. V1.0.2 内置更稳健的 scikit-image 兼容补丁：自动适配 multichannel/channel_axis，并只传递当前 gaussian() 支持的参数。
...
SCRIPT_VERSION = "1.0.4-A0C-PARALLEL-ATOMIC-REPRO"
...
DEFAULT_IMGSZ = 640
DEFAULT_BATCH = 8
DEFAULT_CORRUPTION_WORKERS = 12
DEFAULT_PREFETCH_IMAGES = 96
DEFAULT_CORRUPTION_SEED = 3407
DEFAULT_CONF = 0.001
DEFAULT_IOU_NMS = 0.7
DEFAULT_MAX_DET = 300
DEFAULT_DEVICE = "0"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--corruption-workers",
        type=int,
        default=DEFAULT_CORRUPTION_WORKERS,
        help="CPU processes used to generate corruptions in parallel",
    )
    p.add_argument(
        "--prefetch-images",
        type=int,
        default=DEFAULT_PREFETCH_IMAGES,
        help="Maximum number of corrupted images kept in RAM per CPU window",
    )
    p.add_argument(
        "--corruption-seed",
        type=int,
        default=DEFAULT_CORRUPTION_SEED,
        help="Base seed for deterministic per-image corruption generation",
    )
...
def _install_imagecorruptions_compat_worker():
    """Install imagecorruptions/skimage compatibility inside a CPU worker."""
    alias_pairs = {"int": int, "float": float, "bool": bool, "object": object}
...
_WORKER_CORRUPT_FN = None


def _deterministic_corruption_seed(
    base_seed: int,
    image_id: str,
    corruption_name: str,
    severity: int,
) -> int:
    """Stable uint32 seed independent of Python hash randomization."""
    payload = (
        f"{int(base_seed)}|{image_id}|{corruption_name}|{int(severity)}"
    ).encode("utf-8")
    digest = hashlib.sha256(payload).digest()
    return int.from_bytes(digest[:4], byteorder="little", signed=False)


def _guard_single_a0c_process(results_root: Path) -> Path:
    """
    Prevent simultaneous A0-C scripts from writing the same cache.

    Detects other A0-C runner processes and also uses an exclusive run lock.
    Stale same-host locks are reclaimed automatically.
    """
    current_pid = os.getpid()

    probe = run_cmd(
        ["pgrep", "-af", "run_v10_A0C_HELMET_C_BASELINE_FORMAL"]
    )
    others = []
    ...
    if others:
        raise FormalA0CError(
            "Another A0-C process appears to be running. "
            "Stop it first to prevent cache races:\n"
            + "\n".join(others)
        )

    lock_path = results_root / "A0C_RUN.lock"
    ...
    if lock_path.exists():
        ...
        try:
            ...
            old_pid = int(obj.get("pid", -1))
            old_host = obj.get("hostname")
            if (
                old_host == hostname
                and old_pid > 0
                and Path(f"/proc/{old_pid}").exists()
            ):
                stale = False
        except Exception:
            stale = True
        ...
        if not stale:
            raise FormalA0CError(
                f"Active A0-C run lock exists: {lock_path}"
            )
    ...
    def _release_lock():
        try:
            if lock_path.exists():
                obj = json.loads(lock_path.read_text(encoding="utf-8"))
                if int(obj.get("pid", -1)) == current_pid:
                    lock_path.unlink(missing_ok=True)
        except Exception:
            pass
...
def _corruption_worker(task):
    """Read one image and generate one corrupted RGB uint8 image on CPU."""
    global _WORKER_CORRUPT_FN
    image_path_str, image_id, corruption_name, severity, deterministic_seed = task
    from PIL import Image as PILImage

    np.random.seed(int(deterministic_seed))
    random.seed(int(deterministic_seed))

    # Install/import the compatibility-patched corruption function only once
    # per spawned worker process, not once per image.
    if _WORKER_CORRUPT_FN is None:
        _WORKER_CORRUPT_FN = _install_imagecorruptions_compat_worker()
    corrupt_fn = _WORKER_CORRUPT_FN

    with PILImage.open(image_path_str) as im:
        image_np = np.asarray(im.convert("RGB"), dtype=np.uint8)
    ...
    if out.shape[-1] == 4:
        out = out[..., :3]
    return image_id, np.ascontiguousarray(out)
...
def import_runtime():
    ...
    # scikit-image compatibility patch:
    # imagecorruptions==1.1.2 uses gaussian(..., multichannel=True)
    # in glass_blur, while recent scikit-image uses channel_axis instead.
    # Patch only the gaussian symbol used inside imagecorruptions so the
    # corruption formula and severity parameters remain unchanged.
    try:
        import imagecorruptions.corruptions as ic_corruptions
        from skimage.filters import gaussian as skimage_gaussian
        ...
        _gaussian_sig = inspect.signature(skimage_gaussian)
        _gaussian_params = set(_gaussian_sig.parameters)
        ...
        def _gaussian_compat(
            ...
            **kwargs,
            ...
            #   output argument was also removed in recent versions.
            if multichannel is not None and channel_axis is None:
                channel_axis = -1 if bool(multichannel) else None

            call_kwargs = {}

            # Pass only arguments actually supported by the installed
            # scikit-image gaussian() signature.
            candidates = {
                "sigma": sigma,
                "mode": mode,
                "cval": cval,
            ...
            if "channel_axis" in _gaussian_params:
                call_kwargs["channel_axis"] = channel_axis
            elif "multichannel" in _gaussian_params and multichannel is not None:
                call_kwargs["multichannel"] = bool(multichannel)

            # Only pass output when the installed version supports it and
            # the caller explicitly supplied a non-None output.
            if output is not None and "output" in _gaussian_params:
                call_kwargs["output"] = output

            # Forward any extra keyword only if supported.
            for key, value in kwargs.items():
                if key in _gaussian_params:
                    call_kwargs[key] = value
        ...
        raise FormalA0CError(
            "Failed to install the scikit-image compatibility patch "
            "required by imagecorruptions glass_blur."
        ) from e
...
def condition_protocol(
    ...
    return {
        ...
        "corruption_seed_base": int(corruption_seed),
        "corruption_seed_scheme": (
...
def generate_condition_predictions(
    ...
    """Parallel CPU corruption generation + bounded GPU inference."""
    # Atomic streaming cache. Write to a unique partial gzip and publish
    # with os.replace() only after all test images are successfully processed.
    for stale_partial in cache_path.parent.glob(
        f".{cache_path.name}.partial.*.gz"
    ):
        stale_partial.unlink(missing_ok=True)

    partial_cache = cache_path.parent / (
        f".{cache_path.name}.partial.pid{os.getpid()}.gz"
    )
    ...
    processed = 0
    max_success_batch = 0
    cpu_generate_seconds = 0.0
    gpu_predict_seconds = 0.0

    prefetch_images = max(int(requested_batch), int(prefetch_images))
    append_log(
        progress_log,
        f"START {corruption_name} severity={severity} workers={corruption_workers} prefetch={prefetch_images}",
    )

    with gzip.open(partial_cache, "wt", encoding="utf-8", compresslevel=6) as fout:
        for window_start in range(0, len(image_manifest), prefetch_images):
            rows_window = image_manifest[window_start: window_start + prefetch_images]
            tasks = [
                (
                    row["image_path"],
                    row["image_id"],
                    corruption_name,
                    int(severity),
                    _deterministic_corruption_seed(
                        corruption_seed,
                        row["image_id"],
                        corruption_name,
                        int(severity),
            ...
            t_cpu = time.time()
            generated = list(executor.map(_corruption_worker, tasks, chunksize=1))
            cpu_generate_seconds += time.time() - t_cpu
            generated_by_id = {image_id: arr for image_id, arr in generated}
            arrays_window = [generated_by_id[row["image_id"]] for row in rows_window]

            for sub_start in range(0, len(rows_window), requested_batch):
                rows = rows_window[sub_start: sub_start + requested_batch]
                arrays = arrays_window[sub_start: sub_start + requested_batch]
                ...
                gpu_predict_seconds += time.time() - t_gpu
                max_success_batch = max(max_success_batch, success_batch)
            ...
            del generated, generated_by_id, arrays_window
            gc.collect()

            elapsed = time.time() - start
            rate = processed / max(elapsed, 1e-9)
            eta_min = (len(image_manifest) - processed) / max(rate, 1e-9) / 60.0
            append_log(
                progress_log,
                f"{corruption_name} s{severity}: {processed}/{len(image_manifest)} "
                f"rate={rate:.2f} img/s ETA={eta_min:.1f} min",
            )

    if processed != len(image_manifest):
        raise FormalA0CError(
            f"Condition incomplete before cache finalization: "
    ...
    meta = {
        ...
        "corruption_seed_base": int(corruption_seed),
        "cpu_generate_seconds": cpu_generate_seconds,
        "gpu_predict_seconds": gpu_predict_seconds,
        "wall_seconds": elapsed,
        "wall_minutes": elapsed / 60.0,
        "effective_images_per_second": processed / elapsed if elapsed > 0 else None,
        "completed": now_iso(),
    }
    write_json(meta_path, meta)
    append_log(
        progress_log,
        f"DONE {corruption_name} severity={severity} time={elapsed/60.0:.2f} min "
        f"cpu={cpu_generate_seconds/60.0:.2f} min gpu={gpu_predict_seconds/60.0:.2f} min "
        f"rate={processed/max(elapsed,1e-9):.2f} img/s preds={total_preds}",
    )
    return meta
...
def main() -> int:
    ...
    run_lock_path = _guard_single_a0c_process(results_root)
    ...
    print(f"corruption workers   : {args.corruption_workers}")
    print(f"prefetch images      : {args.prefetch_images}")
    print(f"corruption seed      : {args.corruption_seed}")
    ...
    protocol = {
        ...
        "inference": {
            ...
            "corruption_seed_base": args.corruption_seed,
            "corruption_seed_scheme": (
                "sha256(base_seed|image_id|corruption|severity)->uint32"
            ),
            "cpu_parallel_backend": "ProcessPoolExecutor(spawn)",
    ...
    if args.corruption_workers < 1:
        die("--corruption-workers must be >= 1")
    if args.prefetch_images < 1:
        die("--prefetch-images must be >= 1")

    # Spawn avoids inheriting the already-created CUDA context into CPU workers.
    spawn_ctx = mp.get_context("spawn")
    executor = ProcessPoolExecutor(
        max_workers=int(args.corruption_workers),
    ...
    for corr in selected_corruptions:
        for severity in selected_severities:
            ...
            meta = generate_condition_predictions(
                ...
                prefetch_images=int(args.prefetch_images),
